...
- **Single, High-Fidelity CSV**: A standard comma-separated values file saving clinical metrics for easy copy-pasting or direct import into spreadsheet engines (e.g. Microsoft Excel, Google Sheets). All cells are completely sanitized of HTML elements and formatted with clean, standard multi-line values (using standard `\n` and plain text bullets `- `) to maintain high data fidelity.
- **Excel CSV Safety / Sanitization**: Sanitizes potential formula injection attempts by automatically prepending a single quote (`'`) to cell values starting with `=`/`+`/`@`.
...
- **Unified 12-Column Schema**:
  1. `Patient ID`
  2. `Patient Name`
  3. `Department` (case department)
  4. `CPT/HCPC Code` (resolved HCPC J/Q or CPT, dynamically applying HCPC vs CPT prefixes across all clinical metrics fields depending on the code structure)
  5. `Procedure/Medicine Name`
  6. `Provider` (requesting provider from `pa_request.requesting_provider` or PCP)
  7. `Insurance Type` (plan type from persona `payer.plan_type`)
  8. `Policy Name` (plan name from persona `payer.plan_name`)
  9. `extraction expectation` (corresponds to details_from_extraction)
  10. `likelihood expectations` (corresponds to likelihood_without_documents)
  11. `attachments` (attached clinical documents list)
